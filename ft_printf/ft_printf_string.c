...
/*   Updated: 2017/12/02 17:28:33 by fle-roy          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		print_string(int fd, t_ptf_toprint format, t_ptf_param p, va_list ap)
{
	...
	if (p.precision && len)
		res += ft_putstr_limit(fd, str, p.precision < 0 ? 0 : p.precision);
	else if (!p.precision && !ft_strcmp(str, "(null)") && !p.zero)
		res += ft_putstr_limit(fd, " ", 0);
	if (p.minus && p.padding > 0)
		res += print_padding(fd, (p.zero ? '0' : ' '), p.padding -
		(p.precision <= 0 ? len : p.precision));
	return (res);
...
int		print_pourcent(int fd, t_ptf_toprint format, t_ptf_param param, va_list ap)
{
	int res;
