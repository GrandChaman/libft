...
/*   Updated: 2017/12/02 17:30:18 by fle-roy          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	va_list ap)
{
	...
	res = 0;
	if (format.len)
		res += ft_putstr_limit(m[0], format.str, format.len);
	n = extract_nb(p, ap);
	istr = ft_itoa_base(n, 16);
	if (m[1] == LOWER || m[1] == POINTER)
		ft_strtolower(istr);
	len = ft_strlen(istr);
	p.padding -= (m[1] == POINTER || (p.hashtag && (n || m[1] == POINTER)) ? 2 : 0);
	if ((m[1] == POINTER || (p.hashtag && n)) && p.zero)
		res += ft_putstr_limit(m[0], (m[1] == LOWER || m[1] == POINTER ? "0x" : "0X"), 2);
	res += handle_padding(m[0], p, (n || p.precision ? len : -1), BEFORE);
	if ((m[1] == POINTER || (p.hashtag && n)) && !p.zero)
		res += ft_putstr_limit(m[0], (m[1] == LOWER || m[1] == POINTER ? "0x" : "0X"), 2);
	res += (p.precision > 0 ? print_padding(m[0], '0', p.precision - len) : 0);
	if (n || p.precision)
		res += ft_putstr_limit(m[0], istr, len);
