...
/*   Updated: 2018/05/23 17:54:57 by fle-roy          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (rgxp->cursor + 1 < rgxp->dbuf.cursor + 1 &&
		rgxp->dbuf.buf[rgxp->cursor + 1] > 0 &&
		ft_isquantifier(rgxp->dbuf.buf[rgxp->cursor + 1]))
	{
		rgxp->cursor++;
		*pfunc = *cfunc;
		*cfunc = ft_rgxp_get_pfunc(*rgxp);
	}
}
